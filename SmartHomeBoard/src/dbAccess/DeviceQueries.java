...
public class DeviceQueries {
	...
	public void registerDevice() throws SQLException {
		...
		// System.out.println("insert into Sensor values ('"+
		// device.getName().toString() +"','3','"+
		...
		try {
			String statement = "insert into Sensor values ('" + device.getName().toString() + "','" + assignedID + "','"
					+ String.valueOf(device.getSerialNumber()) + "','" + String.valueOf(device.getPortNumber())
					+ "','127.0.0.1','electronics',true,true,true,null)";
	...
	public ResultSet getInfo() throws SQLException {
		...
		ResultSet result = databaseConnection
				.executeQuery("select * from Sensor where deviceID = " + String.valueOf(device.getId()));
		return result;
	}
	...
	public ResultSet getBoardDevices() throws SQLException {
		...
		databaseConnection = new DatabaseConnection();
		ResultSet result = databaseConnection
				.executeQuery("select * from Sensor where BoardID = " + String.valueOf(device.getBoardID()));
	...
	public void removeDevice() throws SQLException {
		...
		databaseConnection = new DatabaseConnection();
		databaseConnection
				.executeUpdate("UPDATE Sensor SET boardID = NULL WHERE deviceID = " + String.valueOf(device.getId()));
	}
